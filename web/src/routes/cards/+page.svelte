<script lang="ts">
	...
	// Inline cycle editing: only one card open at a time, explicit Save (no auto-submit).
	let editingBank = $state<string | null>(null);
	let editMode = $state<'monthly' | 'days'>('monthly');
	function startEdit(card: (typeof data.cards)[number]) {
		editingBank = card.bank;
		editMode = card.cycle_type === 'days' ? 'days' : 'monthly';
	}
...
<!-- Segmented mode toggle (buttons, not a dropdown) -->
{#snippet modeToggle(active: 'monthly' | 'days', select: (m: 'monthly' | 'days') => void)}
	<div class="inline-flex rounded-[9px] border border-border bg-surface p-0.5 text-xs">
		<button
			type="button"
			onclick={() => select('monthly')}
			class="cursor-pointer rounded-md px-2.5 py-1 font-medium transition-colors {active === 'monthly'
				? 'bg-accent text-white'
				: 'text-muted hover:text-text'}">Day of month</button
		>
		<button
			type="button"
			onclick={() => select('days')}
			class="cursor-pointer rounded-md px-2.5 py-1 font-medium transition-colors {active === 'days'
				? 'bg-accent text-white'
				: 'text-muted hover:text-text'}">Every N days</button
		>
	</div>
{/snippet}
...
{#if owner}
	<form method="POST" action="?/saveCard" use:enhance class="panel mb-6 flex flex-wrap items-end gap-3 p-4">
		...
		<input type="hidden" name="cycle_type" value={newMode} />
		{#if newMode === 'monthly'}
			<label class="flex flex-col gap-1 text-xs text-muted">
				Day
...
<div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
	{#each data.cards as card (card.bank)}
		...
		<section class="panel p-5">
			<div class="mb-3 flex items-start justify-between gap-3">
				<div class="min-w-0">
					...
					<div class="mt-0.5 flex flex-wrap items-center gap-2 text-xs text-muted">
						...
						{#if owner && editingBank !== card.bank}
							<button
								type="button"
								onclick={() => startEdit(card)}
								class="cursor-pointer rounded border border-border px-1.5 py-0.5 font-medium text-muted hover:border-accent hover:text-text"
								>Edit cycle</button
							>
						{/if}
					</div>
			...
			<!-- cycle editor (explicit Save; nothing writes until you click Save) -->
			{#if owner && editingBank === card.bank}
				<form
					method="POST"
					action="?/saveCard"
					...
					class="mb-3 flex flex-wrap items-end gap-2 rounded-[10px] border border-border bg-bg p-3"
				>
					<input type="hidden" name="bank" value={card.bank} />
					<input type="hidden" name="name" value={card.name ?? ''} />
					<input type="hidden" name="card_last4" value={card.card_last4 ?? ''} />
					<input type="hidden" name="cycle_type" value={editMode} />
					<div class="flex flex-col gap-1 text-xs text-muted">
						Cycle mode
						{@render modeToggle(editMode, (m) => (editMode = m))}
					</div>
					{#if editMode === 'monthly'}
						<label class="flex flex-col gap-1 text-xs text-muted">
							Day
							<input name="cycle_start_day" type="number" min="1" max="28" value={card.cycle_start_day || 1} class="w-20 rounded border border-border bg-surface px-2 py-1 text-sm tabular-nums text-text" />
						</label>
					{:else}
						<label class="flex flex-col gap-1 text-xs text-muted">
							Length (days)
							<input name="cycle_length_days" type="number" min="1" value={card.cycle_length_days ?? 30} class="w-24 rounded border border-border bg-surface px-2 py-1 text-sm tabular-nums text-text" />
						</label>
						<label class="flex flex-col gap-1 text-xs text-muted">
							Cycle start date
							<input name="cycle_anchor" type="date" value={card.cycle_anchor ?? ''} class="rounded border border-border bg-surface px-2 py-1 text-sm text-text" />
						</label>
					{/if}
					<button type="submit" class="cursor-pointer rounded-md bg-accent px-3 py-1.5 text-sm font-semibold text-white hover:brightness-105">Save</button>
					<button type="button" onclick={() => (editingBank = null)} class="cursor-pointer rounded-md border border-border px-3 py-1.5 text-sm text-muted hover:text-text">Cancel</button>
				</form>
			{/if}
